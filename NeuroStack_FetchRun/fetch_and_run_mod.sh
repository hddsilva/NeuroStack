...
#Original version had 'which' commands below but amazon linux doesn't have the 
#which binaries
...
fetch_and_run_script () {
  ...
  chmod 777 "${TMPFILE}" || error_exit "Failed to chmod script."
...
fetch_and_run_zip () {
  ...
  chmod 777 "${script}" || error_exit "Failed to chmod script."
